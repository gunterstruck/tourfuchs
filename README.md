...
TourFuchs beantwortet die drei Kernfragen im Vertriebsalltag:

1. **„Wo sind meine Kunden?“** – Excel-Liste hochladen, fertig. Kunden erscheinen
   auf der Karte; Vertriebsbezirke bleiben als räumliche Orientierung sichtbar.
...
| Bereich | Funktion |
|---|---|
| 🎬 **Onboarding & Live-Demos** | Ruhiger Erststart: leere Deutschlandkarte, die Beispielkunden erscheinen von selbst mit „Klick-mich-an"-Hinweis. Solange nur Beispieldaten laufen, begrüßt bei **jedem Start** eine ruhige, nicht-blockierende Karte mittig über der Landkarte („Willkommen bei TourFuchs“) mit zwei gleichwertigen Wegen: **▶ TourFuchs in Aktion sehen** (startet die Live-Demos direkt als Schleife, mit Musik) und **📂 Eigene Daten laden**, dazu die Quittung „Verstanden – erst umsehen“ für diesen Besuch. Der Beispieldaten-Streifen im Panel bietet dieselben zwei Wege dauerhaft an (**▶ In Aktion sehen** · **📂 Eigene Daten laden**) und dazu **🎬 Alle Demos** für den gezielten Einstieg über die Übersicht. Tut niemand etwas, startet die Vorführung nach **10 Sekunden** von selbst (der Knopf füllt sich sichtbar); die erste Bedienung irgendwo in der App bricht das ab, gezählt wird nur bei sichtbarem Tab. Mehrere geführte **Live-Demos** (Geister-Cursor bedient die echte App, je ~30–60 s); am Schreibtisch beginnt die Reihe mit dem Film **„Vom Bezirk zum Kunden“** (ganz Deutschland → eigener Bezirk → Kundenkachel), danach Tour, Lasso und Briefing; die Verschlüsselung kommt zum Schluss. Der schnellste Weg zu eigenen Daten (Liste aus Excel einfügen, ohne echte Daten anzufassen) hat einen eigenen Film. Dazu eine mitlaufende **„Erste Schritte"-Checkliste**, die einklappt, sobald der Nutzer erkennbar arbeitet. Die drei Angebote treten dabei **nacheinander** auf, nicht übereinander: Solange die Hinweiskarte im Bild steht, wartet die Checkliste als schmaler Chip und der Beispieldaten-Streifen zeigt nur seinen Hinweis, nicht noch einmal denselben Knopf – ist die Karte quittiert, klappt die Checkliste auf |
...
| 🛡️ **Sichere Demo** | Eindeutig markierte Beispielkunden an echten Straßen (nur der Straßenname, nie eine Hausnummer; einmal aus OpenStreetMap vorberechnet mit `tools/demo-streets.mjs`, zur Laufzeit keine Anfrage), reservierte Drama-Rufnummern und `example.com`-Adressen; Telefon, E-Mail und Briefing werden lokal simuliert, Exporte als Demo gekennzeichnet |
| 📍 **Verortung** | Sofort über PLZ-Koordinaten (offline, ohne API-Schlüssel); optional adressgenau über OpenStreetMap/Nominatim. Nach einem Import mit Straßen fragt TourFuchs **auf jedem Gerät einmal** nach („Ja, immer genau verorten“ · „Nein, PLZ reicht“) und sagt dabei, was übertragen wird: nur Straße, PLZ und Ort. Bei Ja läuft die Verortung im Hintergrund – nach Importen, beim Start und nach einem Funkloch –, mit Fortschrittsanzeige und „Anhalten“; änderbar unter ⓘ Info. Am Schreibtisch bleibt zusätzlich der Knopf „Adressen exakt verorten“ |
| 🎚️ **Eine Ansicht, voller Umfang** | Den Umschalter „Basis \| Profi" gibt es seit dem 26.09.2026 nicht mehr: Desktop und Handy zeigen immer den vollen Umfang – dieselben Knöpfe, die die Live-Demos vorführen (etwa „🏁 Als Ziel" und „Entlang der Tour"). Am Handy gehört die frühere Pillen-Zeile wieder der Karte. Gebietsplanung und Service bleiben optionale Module |
| 🗺️ **Gebiete als Orientierung** | Vertriebsbezirk, Bezirksfarben und operative Filter bleiben im Außendienst sichtbar. Die kompakten Gebietskacheln sind anklickbar und öffnen eine große Detailkarte mit Kunden, Umsatz, Durchschnitt, Teilgebieten und stärksten Standorten; von dort lässt sich das gesamte Gebiet auf der Karte fokussieren. Vollständige Landkreis-/PLZ-Werkzeuge, strategische Auswertungen und Änderungen gehören zum optionalen Modul **„Gebietsplanung & Gebietsmanagement"** |
| 📋 **Pillen in der Gebietsplanung** | Unten über der Karte zwei Knöpfe für den Vertriebsbeauftragten in seinem Gebiet: **📋 Briefing für mein Gebiet** (die gerade sichtbaren Kunden – Überfällige zuerst, dann bald Fällige, dann die am längsten nicht Besuchten – als Briefing mit Prompt für die freigegebene Firmen-KI) und **⬇ Gebiet exportieren** (dieselben Kunden als Excel mit Kundennummer und Besuchsstand, Dateiname mit dem Bezirk – zum Ablegen oder für den CRM-Import). Beide folgen Bezirks- und Umsatzfilter. Vergleich und Simulation bleiben bewusst im Panel. Am Schreibtisch mit eigenen Daten zusätzlich **🎬 Live-Demos** mit Zähler für ungesehene Demos |
| ✏️ **Gebiets-Editor (optionales Modul)** | Nach Aktivierung im Gebiets-Popup „Kunden dieses Gebiets umordnen": Kundenliste des Gebiets mit Checkboxen & Filter, ausgewählte Kunden (oder das ganze Gebiet) einem anderen Vertriebsbezirk oder einer Gruppe zuweisen – auch gemischte Gebiete gezielt aufteilen; wirkt sofort auf der Karte, mit **Rückgängig**. Der Editor **liegt neben der Karte statt darüber**: Sie bleibt sichtbar und schiebbar, während man umordnet – man sieht ja erst dort, welcher Bezirk nebenan liegt –, und ein Klick auf das nächste Gebiet füllt den Editor neu, statt an ihm abzuprallen |
...
| 👥 **Filter** | **Vertriebsbezirk** ist die führende Ebene. Eine Auswahl isoliert auf der Gebietskarte auch die passenden Flächen, Beschriftungen und Legendeneinträge. Ein aktivierbares **Umsatzintervall von–bis** blendet Kleinst-, Null- oder unbekannte Umsätze gezielt aus. Zusätzlich lässt sich festlegen, ab wie vielen sichtbaren Kunden eine Landkreis-/PLZ-Fläche überhaupt Farbe erhält; dünn besetzte Ausreißer bleiben neutral. Die Gebietsflächen lassen sich außerdem komplett ausblenden |
| 🚗 **Tour** | **Ohne Vorabfrage startklar:** geplant wird ab Werk über **alle Vertriebsbezirke** – eine schmale Zeile („🗺️ Bezirk: Alle Bezirke · ändern ▸") zeigt den Stand und schränkt auf Wunsch ein; bei nur einem Bezirk entfällt sie ganz. Startpunkt = eigener GPS-Standort, ein Kunde **oder ein Ort** – dasselbe Suchfeld findet Kunden, Ortsnamen und Postleitzahlen (und erkennt eingefügte Koordinaten). **„📌 Auf der Karte setzen"** ergänzt die exakte Wahl per Fingertipp oder ziehbarem Pin: Ladestation, Büro oder Hotel werden benannt, lokal gemerkt und als eigene violette Kartenpunkte wiedergefunden. Straße und Hausnummer bleiben freiwillige Beschriftung; bei einem Karten-Pin verwendet Planung und Google-Maps-Navigation immer die bewusst gesetzte Koordinate. Eigene Orte liegen im Tresor und sind Wegpunkte, **keine Kunden**: Sie zählen in keiner Kundenliste, Umsatzsumme oder Gebietsauswertung mit. Das **Ziel** nimmt denselben Weg – für die Rückgabestation am Abend. Vorschläge „Wen könnte ich in der Nähe noch besuchen?“ (Umkreis einstellbar); Tourenoptimierung (kürzeste Strecke, Nearest-Neighbor + 2-Opt); Übergabe an Google Maps zur Navigation. Unterwegs wird jeder Stopp mit „✓ Heute" abgehakt (am Handy per Tipp auf den Tour-Punkt), und der **Feierabend-Rückblick** zeigt abends den Tag in Zahlen: Besuche, geschätzte Strecke, abgearbeitete Überfällige, offen Gebliebenes – als Text kopierbar. Am Handy sind Startpunkt · Vorschläge · Meine Tour ein aufgeräumtes **Akkordeon** (genau eine Gruppe offen); darüber steht im selben Gewand die eingeklappte Karte **„In der Nähe"**, die zugeklappt Anzahl und Entfernung des nächsten Kunden nennt („12 · ab 1,1 km") und aufgeklappt die fünf nächstgelegenen zeigt. Ein kleiner schwebender **Fuchs-Knopf** schlägt kontextabhängig den nächsten Schritt vor (Kunden in der Nähe → Tour ab hier planen → Route auf die Karte) |
...
| 🧭 **Mehrkunden-Briefing** | „Wen zuerst?" unter den Tourvorschlägen und unter „In der Nähe": operativer Basis-Weg für eine geografisch sinnvolle Kundenauswahl. Der Prompt enthält je Kunde nur Name, Kundennummer, PLZ/Ort, Fälligkeit und letzten Besuch – kein Umsatz, keine Kontaktdaten, keine Straße –, ist auf 12 Kunden begrenzt und verlangt eine Besuchsreihenfolge statt eines weiteren Berichts |
| 🖊️ **Lasso auf der Karte** | Eine Fläche frei umfahren statt einen Regler zu schieben – der Knopf steht gleichrangig neben „Kunden in meiner Nähe“. Die Karte friert ein, die Spur wächst mit dem Finger mit, die Treffer leuchten auf, und eine **Auswahlkarte im Gewand der Kundenkarte** nennt Anzahl, fällige Kunden, Umsatz und Orte – von dort direkt ins Mehrkunden-Briefing. Jede Zeile trägt ein **Häkchen**: „Alle zur Tour" gab es schon, mit Häkchen heißt der Knopf „🚩 3 zur Tour" und übernimmt genau die – die Auswahl bleibt dabei liegen. Trifft auch **unrunde** Gebiete (Gewerbegebiet, Flussseite, Autobahnkorridor), die ein Umkreis prinzipiell verfehlt. Auf Handy und Tablet-Hochkant schiebt sich das Blatt dafür auf Guckhöhe |
| 🛡️ **Service-Vertragsradar** | Separater Excel-/CSV-Import für Vertragsstände aus SAP, SieSales/Salesforce oder weiteren Quellen. Exakter Kundenabgleich per Kundennummer, Quellenalter, Handlungsfristen, Vertragswert, SLA und Verantwortliche. Der Service-Modus ist ein **optionales Modul** – standardmäßig ausgeblendet und unter **⚙️ Optionale Module** aktivierbar |
...
| 📐 **Zwei Gesichter, ein Außendienstkern** | Am **Schreibtisch** liegt das Panel seitlich, am **Handy** und hochkanten Tablet unten als Blatt. Unterwegs stehen Karte, Kunden, Briefing und Tour im Mittelpunkt; strategisches Gebietsmanagement wird am Desktop konfiguriert und genutzt. **Drehen ändert nur die Geometrie** und verwirft nie laufende Arbeit. Quer kehrt die Seitenleiste zurück. Unterwegs gibt es **keine Reiterleiste**: Dort steht ein Bereich, und das Blatt ist der Schalter – unten die Karte, oben die Tour. Ziehen ändert die Höhe, ein **Tipp** auf den Griff klappt ein und aus, „☰" tut dasselbe. Die installierte App ist **nicht mehr aufs Hochformat gesperrt** |
...
Die geprüfte Excel-Vorlage mit Auswahllisten und Feldbeschreibung sowie ein UTF-8-CSV-Beispiel stehen unter **Service → Verträge** bereit. Das vollständige Schema und die Importregeln beschreibt [Serviceverträge in TourFuchs importieren](./docs/servicevertraege-import.md).
...
- Optionale Straßenrouten (Routenlinie und Korridor-Vorschläge) senden **nach ausdrücklicher Zustimmung** die Koordinaten von Start und Tour-Stopps an OSRM (`router.project-osrm.org`) – keine Namen oder sonstigen Kundendaten. Ohne Zustimmung rechnet die App mit der Luftlinie, komplett offline. Die einzige Ausnahme ist die Tour-Demo: Ihre Straßenroute ist vorberechnet (`tools/demo-route.mjs`) und kommt vom eigenen Server – ohne Anfrage an OSRM.
...
- [Kunden- und Mehrkunden-Briefing](./docs/kundenbriefing.md) – Ablauf, Inhalt der Prompts, Wahl des Assistenten und Prüfschritte
...
danach wieder schieben lässt. Dazu der Rückweg: ob ein Daumen die
...
Reiter) und je Reiter, getrennt nach Modus – und prüft zwei Regeln aus
